...
export const ReactServer = {
	renderRSC: async (request: Request, component: React.ReactNode) => {
		const rscModule = await import.meta.viteRsc.loadModule<typeof import('./render-rsc.tsx')>("rsc", "index")
		return rscModule.renderRSC(request, component);
	},
	renderSSR: async (rscStream: ReadableStream<Uint8Array>, options: {
		formState?: ReactFormState;
		nonce?: string;
		debugNojs?: boolean;
	}) => {
		const ssrModule = await import.meta.viteRsc.loadModule<typeof import('./render-ssr.tsx')>("ssr", "index")
